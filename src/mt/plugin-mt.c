...
static void
procs2_mt_callback(int version, lcb_loop_procs *loop_procs,
...
{
    ev_procs->create = mt_event_new;
    ev_procs->destroy = mt_event_free;
    ev_procs->watch = mt_event_update;
    ev_procs->cancel = mt_event_cancel;

    timer_procs->create = mt_timer_new;
    timer_procs->destroy = mt_timer_free;
    timer_procs->schedule = mt_timer_schedule;
    timer_procs->cancel = mt_timer_cancel;

    loop_procs->start = mt_run_loop;
    loop_procs->stop = mt_stop_loop;
    loop_procs->tick = NULL;

    *iomodel = LCB_IOMODEL_EVENT;
    wire_lcb_bsd_impl2(bsd_procs, version);

    bsd_procs->sendv = sendv_wrap;
    bsd_procs->recvv = recvv_wrap;
...
lcb_create_mtio_io_opts(int version, lcb_io_opt_t *io, void *arg)
{
    ...
    ret->v.v3.get_procs = procs2_mt_callback;
