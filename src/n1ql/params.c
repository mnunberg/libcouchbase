...
lcb_error_t
lcb_n1p_synctok(lcb_N1QLPARAMS *params, const lcb_SYNCTOKEN *stok, lcb_U16 vbid)
{
    size_t np;
    ...
    np = sprintf(params->scanvec.base + params->scanvec.nused, SCANVEC_BASEFMT,
        vbid,
        (unsigned long long)LCB_SYNCTOKEN_ID(stok),
        (unsigned long long)LCB_SYNCTOKEN_SEQ(stok));
