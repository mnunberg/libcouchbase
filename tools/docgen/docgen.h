...
public:
    ...
    gen_graded_sizes(uint32_t minsz, uint32_t maxsz, int grades=10) {
...
protected:
    ...
    void initMatches(
        ...
        for (std::vector<TemplateSpec>::const_iterator ii = specs.begin();
                ii != specs.end(); ++ii) {
            Placeholders::Spec cur(ii->term, ii->minval, ii->maxval, ii->sequential);
            pl_specs.push_back(cur);
        }

        for (std::vector<std::string>::const_iterator ii = inputs.begin();
                ii != inputs.end(); ++ii) {
...
private:
    class MyState : public GeneratorState {
    public:
        MyState(const PlaceholderDocGenerator *parent, int total, int cur)
        : m_parent(parent) {
            using namespace Placeholders;
            for (size_t ii = 0; ii < parent->matches.size(); ii++) {
                m_substs.push_back(
                    Substitutions(parent->matches[ii], total, cur));
            }
    ...
    void initJsonPlaceholders(
        ...
        for (size_t ii = 0; ii < documents.size(); ii++) {
            ...
            if (!reader.parse(documents[ii], root)) {
                throw std::string("Couldn't parse one or more documents!");
            }
